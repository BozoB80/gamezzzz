  ...
  const onCheckout = async () => {
    try {
      ...
      if (error.response && error.response.data.includes("is already")) {
        ...
        toast({
          variant: "destructive",
          title: `The game ${gameTitle} in your library!`,
          description: "Please remove it to continue.",
        });
      } else {
    ...
    <div className="mt-16 rounded-lg px-4 py-6 sm:p-6 lg:col-span-5 lg:mt-0 lg:p-8 text-white/90">
      ...
      <div className="mt-6 space-y-4">
        ...
        <div className="flex items-center text-white/60 justify-between pt-4">
          <div className="text-xl font-medium">Sale discount</div>
          <h1 className="text-xl font-medium">- €{totalDiscount.toFixed(2)}</h1>
        </div>
        <div className="flex items-center justify-between border-t border-gray-200 pt-4">
          <div className="text-xl font-medium">Order Total</div>
      ...
      <Button
        ...
        size="lg"
        onClick={onCheckout}
        className="w-full text-xl font-semibold mt-6 py-2 rounded-md"
      >
        Checkout
      </Button>
    </div>
